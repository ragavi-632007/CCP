...
export class DatabaseStorage implements IStorage {
  ...
  async getForumPosts(
    category?: string
  ): Promise<Array<ForumPost & { author: User }>> {
    const query = db
      .select({
        id: forumPosts.id,
  ...
  async getForumComments(
    postId: string
  ): Promise<Array<ForumComment & { author: User }>> {
    return await db
      .select({
        id: forumComments.id,
  ...
  async createForumComment(
    insertComment: InsertForumComment
  ...
  async createGovernmentScheme(
    insertScheme: InsertGovernmentScheme
